  ...
  const handleFetchInfo = async () => {
    ...
    try {
      ...
      try {
        ...
        const playDlResponse = await axios.post(`${API_BASE_URL}/api/info-playdl`, { url });
        setVideoInfo(playDlResponse.data);
      } catch (playDlErr: any) {
        console.log("Fallback 1 failed, trying fallback 2...");
        try {
          // 3. Fallback to simple method
          const simpleResponse = await axios.post(`${API_BASE_URL}/api/info-simple`, { url });
          setVideoInfo(simpleResponse.data);
        } catch (simpleErr: any) {
          console.log("Fallback 2 failed, trying fallback 3...");
          try {
            // 4. Final Fallback (noembed)
            const fallbackResponse = await axios.post(`${API_BASE_URL}/api/info-fallback`, { url });
            setVideoInfo(fallbackResponse.data);
          } catch (finalErr) {
            setError("Unable to process this video. Please ensure the link is correct and accessible.");
          }
        }
      }
    } finally {
    ...
    <main className="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-purple-500 selection:text-white">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6 pt-20 pb-16 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 mb-8 backdrop-blur-md">
            ...
            <span className="text-sm font-medium text-gray-300">Fast & Free Video Downloader</span>
          </div>
          ...
          <p className="text-xl md:text-2xl text-gray-400 mb-12 max-w-2xl mx-auto leading-relaxed">
            Download <span className="text-white font-semibold">YouTube videos</span> and <span className="text-white font-semibold">MP3 audio</span> instantly. No ads, no limits, just flow.
          </p>
          ...
          <div className="max-w-3xl mx-auto bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-2 md:p-3 shadow-2xl shadow-purple-900/20 transform transition-all hover:scale-[1.01]">
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <button
                ...
                disabled={loading}
                aria-label={loading ? "Processing video" : "Start video download process"}
          ...
          <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl">
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
              ...
              <div className="flex-1 w-full text-center md:text-left">
                <h2 className="text-2xl md:text-3xl font-bold mb-4 leading-tight">{videoInfo.title}</h2>
      ...
      <section className="bg-white/5 py-20 border-y border-white/5">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-16">Why Choose VidFlow?</h2>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: FaRocket, title: "Lightning Fast", desc: "Our advanced servers process videos instantly. No waiting queues or slow downloads." },
              { icon: FaShieldAlt, title: "100% Secure", desc: "No data collection, no malicious ads. Your privacy is our top priority." },
              { icon: FaCheckCircle, title: "High Quality", desc: "Download in HD, 1080p, and 4K quality. Crystal clear audio extraction." }
            ].map((feature, idx) => (
              <div key={idx} className="p-8 rounded-2xl bg-white/5 border border-white/10 hover:border-purple-500/50 transition-colors">
      ...
      <section className="max-w-4xl mx-auto px-6 py-20 text-gray-400">
        <div className="prose prose-invert max-w-none">
          <h2 className="text-3xl text-white font-bold mb-6">Best YouTube Downloader and MP3 Converter</h2>
          <p>
            VidFlow is the premier tool for downloading videos from YouTube.
            Whether you need to save a tutorial for offline viewing or convert a music video to MP3 for your playlist,
            VidFlow makes it simple, fast, and free.
          </p>

          <h3 className="text-2xl text-white font-bold mt-12 mb-6">Frequently Asked Questions</h3>
          <div className="space-y-8">
            <div>
              <h4 className="text-xl text-white font-semibold mb-2">Is VidFlow free to use?</h4>
              <p>Yes, VidFlow is completely free. You can download as many videos as you want without any hidden costs.</p>
            </div>
            <div>
              <h4 className="text-xl text-white font-semibold mb-2">Can I download videos on my phone?</h4>
              <p>Absolutely. VidFlow is optimized for all devices including Android, iPhone, Mac, and Windows PC.</p>
            </div>
            <div>
              <h4 className="text-xl text-white font-semibold mb-2">Is it safe?</h4>
              <p>Yes, we do not store any of your data or downloaded files. The connection is secure and private.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-white/10 py-12 text-center text-gray-500 text-sm">
        ...
        <p>Disclaimer: VidFlow is not affiliated with YouTube. Please respect copyright laws.</p>
      </footer>
