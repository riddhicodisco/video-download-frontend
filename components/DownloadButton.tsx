...
export default function DownloadButton({
  ...
  const handleDownload = async (quality: string = "auto") => {
    ...
    try {
      // Try different methods in order - Universal first (works for all users)
      const methods = [
        {
          name: "universal",
          endpoint:
            type === "video"
              ? "/api/download/video-universal"
              : "/api/download/audio-universal",
        },
        {
          name: "ytdl-core",
          endpoint:
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`w-full px-6 py-4 font-bold text-white rounded-xl transition-all transform hover:-translate-y-1 ${
          type === "video"
